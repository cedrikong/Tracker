<?php
  ...
  if($_SERVER["REQUEST_METHOD"]=='GET'){
    if(!isset($_GET['id'])){
      header("location:crud100/projects.php");
      exit;
    }
    $id = $_GET['id'];
    ...
    $sql = "update crud2 set name='$name', email='$email', trans='$trans', phone='$phone', status='$status' where id='$id'";
    $result = $conn->query($sql);
...
<div style="padding:25px; color:white;">
  <h1 class="text-white text-center">  Update Merchant</h1>
...
<label> MATCH TRANSACTION: </label>
 <input type="text" name="trans" value="<?php echo $trans; ?>" class="form-control"> <br>
 ...
 <input type="text" name="phone" value="<?php echo $phone; ?>" class="form-control"> <br>
  ...
  <label> STATUS: </label>
 <select type="text" name="status" value="<?php echo $status; ?>" class="form-control"> 
 <option></option>
...
<option class='red-option'>COMPLETED</option>
 ...
 <div style="height:60px; background-color:rgb(0,0,0,0.23); padding:0.5%; float:left; width:100%; border-top: 2px solid); margin-top:406px; text-align:center; color:gray; font-family:tahoma; font-size: smaller;">Designed by<font color="#5bc2f2"> ariespogi</font> </div>
</body>
</html>